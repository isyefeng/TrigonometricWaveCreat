...
B:直流分量
'''
def CreatSinINTArray(file='SinDataFile.txt', size=100, A=1, B=0, offset=0):
	data = []
	for i in range(size):
		angle = (i*2*3.14159)/size+(2*3.14159*offset)/360
...
B:直流分量
'''
def CreatSinArray(file, size, A, B, offset=0 ):
	data = []
	for i in range(size):
		angle = (i*2*3.14159)/size+(2*3.14159*offset)/360
...
def creatfile(event):
	...
	AngleOffset = int(offset_text.GetValue())			#获取偏移角度
	FILE = file_text.GetValue()
	DataType = radiobox3.GetSelection()
	if DataType == 0:
		start = CreatSinINTArray(FILE, NUM, A, B, AngleOffset)
	elif DataType == 1:
		start = CreatSinArray(FILE, NUM, A, B, AngleOffset)
	if start:
		wx.MessageBox("OK!")
	else:
		wx.MessageBox("ERROR")
...
def DisplaySinWave(event):
	...
	print(DataType)
	if DataType == 0:
		ShowSinWave(CreatSinINTArray(FILE, NUM, A, B, AngleOffset))	
	elif DataType == 1:
		ShowSinWave(CreatSinArray(FILE, NUM, A, B, AngleOffset))	
...
wx.StaticText(frame, label = 'Offset:',pos=(190,60))
...
file_text = wx.TextCtrl(frame, pos = (70,55), size = ( 100,24)) #
offset_text = wx.TextCtrl(frame, pos = (240,55), size = ( 60,24)) #
